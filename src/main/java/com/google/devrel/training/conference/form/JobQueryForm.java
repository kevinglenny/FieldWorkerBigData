...
/**
 * A simple Java object (POJO) representing a query options for Job.
 */
public class JobQueryForm {

    private static final Logger LOG = Logger.getLogger(JobQueryForm.class.getName());
    ...
    public static enum Field {
        ...
        DATE("Date", FieldType.INTEGER),
        PRIORITY("Priority", FieldType.STRING),
        STATUS("Status", FieldType.STRING),
        INVOICE_COST("Cost", FieldType.DOUBLE);
     ...
     */
    public JobQueryForm filter(Filter filter) {
        if (filter.operator.isInequalityFilter()) {
     ...
     */
    @ApiResourceProperty(ignored = AnnotationBoolean.TRUE)
    public Query<Job> getQuery() {
        // First check the feasibility of inequality filters.
        checkFilters();
        Query<Job> query = ofy().load().type(Job.class);
